...
Route::group(['namespace' => 'Api\V1', 'middleware'=>'localization'], function () {
    ...
    Route::post('vodafone_sms',function(Request $request){

        if($request->secret_key == '0CF5E037A4224709A035AF92E4E886BD' && $request->mobile && $request->msg){
            
            if (preg_match('/^(010|012)/', $request->mobile)){
                Helpers::vodafoneSms($request->mobile,$request->msg);
            }
            else{
                   Helpers::sendWhatsapp($request->mobile,$request->msg);
            }
             
                    return json_encode(['status'=> true, 'message'=> 'success'],);
        }
        return json_encode(['status'=> false, 'message'=> 'error'],422);
    });
    ...
    Route::group(['middleware'=>['module-check']], function(){
        ...
        Route::group(['prefix' => 'banners'], function () {
         ...
         Route::group(['prefix' => 'home_banners'], function () {
            Route::get('/', 'BannerController@get_home_banners');
        });
